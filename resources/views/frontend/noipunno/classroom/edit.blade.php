...
@section('content')
    <div class="dashboard-section">
                                            ...
                                            <table class="table np-table">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th scope="col" class="fw-bold">ক্লাস
                                                            {{-- <span class="icon"><img
                                                                    src="{{ asset('frontend/noipunno/images/icons/filter.svg') }}"
                                                                    alt=""></span> --}}
                                                        </th>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td scope="row">
                                                                <div class="action-content">
                                                                    {{-- <h2 class="created-date">{{ date('d M, Y', strtotime( @$item->created_at)) }}</h2> --}}
                                                                    ...
                                                                    {{-- <a class="btn np-delete-btn-small"
                                                                            id="delete" title="Delete"
                                                                            data-id="{{ $item->uid }}"
                                                                            data-token={{ csrf_token() }}
                                                                            data-route="{{ route('noipunno.dashboard.classroom.delete') }}"><i class="fa fa-trash np-delete-btn-small-icon"></i></a> --}}
                                                                    {{-- <button class="btn np-edit-btn-small np-edit-btn-small-lg">
        ...
        <div class="container">
            <div class="row">
                <h2 class="mt-4 mb-2 np-form-title">সেকশন ভিত্তিক বিষয় শিক্ষকের তথ্য সম্পাদনা করুন</h2>
                <div class="col-md-12">
                    <form action="{{ route('noipunno.dashboard.classroom.update', @$class_room->uid) }}" method="POST">
                        <section class="section-teacher-add-form np-input-form-bg mb-3">
                            <div class="container">
                                                ...
                                                <select class="form-select np-teacher-input" name="branch_id" id="branch_id"
                                                    ...
                                                    @foreach ($branches as $item)
                                                        <option value="{{ $item->uid }}"
                                                            {{ @$class_room->branch_id == $item->uid ? 'selected' : '' }}>
                                                            {{ $item->branch_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="input-group" style="pointer-events: none;">
                                                <select class="form-select np-teacher-input" id="class_id"
                                                    ...
                                                    {{-- @foreach ($classes as $item)
                                                        <option value="{{ $item['class_id'] }}" {{ @$class_room->class_id == $item['class_id'] ? 'selected' : '' }}>{{ $item['name_en'] }}
                                                        </option>
                                                        @endforeach --}}
                                                    @foreach (App\Helper\ClassEnum::values() as $key => $value)
                                                        <option value="{{ $key }}"
                                                            {{ @$class_room->class_id == $key ? 'selected' : '' }}>
                                                            {{ $value }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                ...
                                @endforeach --}}
                                    ...
                                    <div class="col-md-4 col-sm-12 mt-3">
                                        <div>
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" value="{{ @$class_room->session_year }}"
                                                    class="form-control np-teacher-input" name="session_year"
                                                    id="session_year" readonly>
                                            </div>
                                    ...
                                    <div class="col-md-4 col-sm-12 mt-3">
                                        <div>
                                            <label for="beiin-psid" class="form-label">শ্রেণী শিক্ষক <span
                                                    class="text-danger">*</span></label>
                                            <div class="input-group">
                                                {{-- @dd($teachers) --}}
                                            ...
                                            <option value="3">Teacher Three</option> --}}
                                                    ...
                                                    @foreach ($teachers as $item)
                                                        <option value="{{ $item->uid }}"
                                                            {{ @$class_room->class_teacher_id == $item->uid ? 'selected' : '' }}>
                                                            {{ $item->name_en }} -
                        ...
                        <section class="np-teacher-add-form np-input-form-bg mb-5">
                            <div class="">
                                <div class="container">
                                    {{-- <h5 class="mb-3">বিষয় ও শিক্ষক যোগ করুণ</h5> --}}
                                    <form>
                                            ...
                                            <div class="col-md-12 col-sm-12 d-flex justify-content-start">
                                                <button type="submit"
                                                    ...
                                                    style="width: fit-content;border: unset;column-gap: 10px;"> <a
                                                        class="np-btn-cancel"
                                                        href="{{ route('noipunno.dashboard.classroom.add') }}">বাতিল
                                                        করুন</a>
                                                    <img src="{{ asset('frontend/noipunno/images/icons/arrow-right.svg') }}"
                                                        alt=""></button>
                                                @if($class_room->session_year == date('Y'))
                                                <button type="submit"
                                                    class="btn btn-primary np-btn-form-submit mx-5 d-flex align-items-center"
                                                    style="width: fit-content;border: unset;column-gap: 10px;">তথ্য হালনাগদ
                                                    করুন
                                                    <img src="{{ asset('frontend/noipunno/images/icons/arrow-right.svg') }}"
                                                        alt=""></button>
                                                @endif
        ...
        $(document).on('change', '#branch_id', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var vselected = "{{ @$class_room->version_id }}";
                    var sselected = "{{ @$class_room->shift_id }}";
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <select name="teacher_ids[${subject.uid}]" class="form-select np-teacher-input select2" aria-label="">
                                            ...
                                            @php
                                                $subject_teachers_all = $subject_teachers->where('teacher_uid', $teacher->uid);
                                                $json_data = json_encode($subject_teachers_all->pluck('subject_uid')->toArray());
                                            @endphp
                                            <option value="{{ $teacher->uid }}" ${('{{ $json_data }}'.includes(subject.uid))?('selected'):('')}>{{ $teacher->name_en }} - {{ @$teacher->pdsid ?? (@$teacher->index_number ?? @$teacher->caid) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
        ...
        $(document).on('change', '#class_id', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var selected = "{{ @$class_room->section_id }}";
                    $('#section_id').html('');
                    if (data.sections) {
