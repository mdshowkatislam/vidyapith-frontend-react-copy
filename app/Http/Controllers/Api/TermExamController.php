...
use App\Http\Requests\TermExam\TermExamStoreRequest;
use App\Http\Requests\TermExam\TermExamUpdateRequest;
use App\Traits\ApiResponser;
use App\Traits\ValidtorMapper;
use App\Services\TermExamService;
use Exception;
...
class TermExamController extends Controller
{
    ...
    private $termExamService;

    public function __construct(TermExamService $termExamService)
    {
        $this->termExamService = $termExamService;
    }
    ...
    public function index()
    {
        try {
            ...
            $classTestList = $this->termExamService->getByEiinId($eiinId);
     ...
     */
    public function store(TermExamStoreRequest $request)
    {
        try {
            ...
            foreach ($request->section_id as $key => $data) {
                ...
                $alreadyExist = $this->termExamService->alreadyExist($payload);
                if(count($alreadyExist)>0) return $this->errorResponse('বিষয়বস্তুর নাম ইতিমধ্যে বিদ্যমান', Response::HTTP_FOUND);

                $term_exam[] = $this->termExamService->create($payload);
            }
            $message = 'প্রান্তিক পরীক্ষা সফলভাবে তৈরি করা হয়েছে।';
            return $this->successResponseWithData($term_exam, $message, Response::HTTP_OK);
        } catch (Exception $e) {
            // return $this->errorResponse($e->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
            $message = 'প্রান্তিক পরীক্ষা তৈরি করা সম্ভব হয় নি।';
    ...
    {
        try {
            $branch = $this->termExamService->getById($uid);
            if($branch){
     ...
     */
    public function update(TermExamUpdateRequest $request)
    {
        try {
            $term_exam = [];
            foreach ($request->section_id as $key => $data) {
                ...
                $term_exam[] = $this->termExamService->update($payload);
            }
            $message = 'প্রান্তিক পরীক্ষা সফলভাবে আপডেট করা হয়েছে।';
            return $this->successResponseWithData($term_exam, $message, Response::HTTP_OK);
        } catch (Exception $e) {
            // return $this->errorResponse($e->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
            $message = 'প্রান্তিক পরীক্ষা আপডেট করা সম্ভব হয় নি।';
            return $this->errorResponse($message, Response::HTTP_FORBIDDEN);
        }
    }
    ...
    {
        $this->termExamService->delete($id);
        return response()->json(['status' => 'success', 'message' => 'প্রান্তিক পরীক্ষা তথ্যটি মুছে ফেলা হয়েছে।']);
    }
}
