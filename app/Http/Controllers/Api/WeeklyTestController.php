...
use App\Http\Requests\WeeklyTest\WeeklyTestStoreRequest;
use App\Http\Requests\WeeklyTest\WeeklyTestUpdateRequest;
use App\Traits\ApiResponser;
use App\Traits\ValidtorMapper;
use App\Services\WeeklyTestService;
use Exception;
...
class WeeklyTestController extends Controller
{
    ...
    private $weeklyTestService;

    public function __construct(WeeklyTestService $weeklyTestService)
    {
        $this->weeklyTestService = $weeklyTestService;
    }
    ...
    public function index()
    {
        try {
            ...
            $classTestList = $this->weeklyTestService->getByEiinId($eiinId);
     ...
     */
    public function store(WeeklyTestStoreRequest $request)
    {
        try {
            ...
            foreach ($request->section_id as $key => $data) {
                ...
                $alreadyExist = $this->weeklyTestService->alreadyExist($payload);
                if(count($alreadyExist)>0) return $this->errorResponse('বিষয়বস্তুর নাম ইতিমধ্যে বিদ্যমান', Response::HTTP_FOUND);

                $class_test[] = $this->weeklyTestService->create($payload);
            }

            $message = 'সাপ্তাহিক পরীক্ষা সফলভাবে তৈরি করা হয়েছে।';
            return $this->successResponseWithData($class_test, $message, Response::HTTP_OK);
        } catch (Exception $e) {
            // return $this->errorResponse($e->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
            $message = 'সাপ্তাহিক পরীক্ষা তৈরি করা সম্ভব হয় নি।';
    ...
    {
        try {
            $branch = $this->weeklyTestService->getById($uid);
            if($branch){
     ...
     */
    public function update(WeeklyTestUpdateRequest $request)
    {
        try {
            $class_test = [];
            foreach ($request->section_id as $key => $data) {
                ...
                $class_test[] = $this->weeklyTestService->update($payload);
            }

            $message = 'সাপ্তাহিক পরীক্ষা সফলভাবে আপডেট করা হয়েছে।';
            return $this->successResponseWithData($class_test, $message, Response::HTTP_OK);
        } catch (Exception $e) {
            // return $this->errorResponse($e->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
            $message = 'সাপ্তাহিক পরীক্ষা আপডেট করা সম্ভব হয় নি।';
            return $this->errorResponse($message, Response::HTTP_FORBIDDEN);
        }
    }
    ...
    {
        $this->weeklyTestService->delete($id);
        return response()->json(['status' => 'success', 'message' => 'সাপ্তাহিক পরীক্ষা তথ্যটি মুছে ফেলা হয়েছে।']);
    }
}
